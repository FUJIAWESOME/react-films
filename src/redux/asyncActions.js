...
import { addFavoriteFilm, deleteFavoriteFilm, loadCredits, loadDetails, loadFavoriteList, loadFilms, loadGenres } from "./actions";
...
export const fetchFilms = (filters) => {
    return dispatch => {
        ...
        const sortByUrl = filters.selectSort === 'Популярности' ? URL.API.POPULAR_FILMS : URL.API.TOP_RATED_FILMS;
        const defaultUrl = `${sortByUrl}&page=${filters.page}`;
        const searchUrl = new window.URL(`${URL.API.SEARCH}${filters.searchText}&include_adult=false&language=ru&page=${filters.page}`);
...
export const fetchFavoriteList = () => {
    return dispatch => {
        fetch(`https://api.themoviedb.org/3/account/${accountId}/favorite/movies`, optionsGET)
        .then(response => response.json())
...
export const changeFavoriteList = (id, isFavorite) => {
    return dispatch => {
        ...
        fetch(`https://api.themoviedb.org/3/account/${accountId}/favorite`, optionsPOST)
        .then(response => response.json())
        .then(response => {
